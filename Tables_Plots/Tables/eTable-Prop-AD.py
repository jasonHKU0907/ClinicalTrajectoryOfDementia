...
dpath1 = '/Volumes/JasonWork/Projects/UKB_DM_Trajectories/Results/AD_Analysis/'
dpath2 = '/Volumes/JasonWork/Projects/UKB_DM_Trajectories/Data/AD_Analysis/Partition_NA80/'
outpath = dpath2
f_df = pd.read_csv(dpath1 + 'PropData1.csv')
f_df = f_df.loc[f_df.Figure3 == 1]
f_df.reset_index(inplace = True)
f_df.drop('index', axis = 1)
f_df['direction'] = f_df.OR_0to5yrs > 1
f_df['direction'] = f_df.direction.astype('int')

f_lst = ['eid', 'Gender', 'case_control', 'BL2AD_yrs'] + f_df.FieldID_full.tolist()
mydf = pd.read_csv(dpath2 + 'PriorAD_0to15yrs.csv', usecols= f_lst)
...
newdf = pd.concat((mydf[['eid', 'case_control', 'BL2AD_yrs']], newdf), axis = 1)

newdf = newdf.loc[newdf.BL2AD_yrs<=15]
newdf.reset_index(inplace = True)
newdf.drop('index', axis = 1, inplace = True)

#newdf['BL2AD_YR'] = pd.cut(newdf.BL2AD_yrs, bins=[0, 3, 5, 6, 7, 8, 9, 10, 11, 12, 20],
#                           labels = [1.5, 4, 5.5, 6.5, 7.5, 8.5, 9.5, 10.5, 11.5, 13.5])
#time_ivs = [1.5, 4, 5.5, 6.5, 7.5, 8.5, 9.5, 10.5, 11.5, 13.5]

newdf['BL2AD_YR'] = pd.cut(newdf.BL2AD_yrs, bins=[0, 5, 10, 15],
                           labels = [2.5, 7.5, 12.5])
...
for tiv in time_ivs:
    tmpdf = newdf.loc[newdf.BL2AD_YR == tiv]
...
tmpdf_prop.to_csv(dpath1 + 'PropData2.csv', index = False)
